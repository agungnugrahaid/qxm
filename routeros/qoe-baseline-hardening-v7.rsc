# qoe-baseline-hardening-v7.rsc (RouterOS v7)
#
...
# --- NTP (v7: primary-ntp/secondary-ntp were replaced by a servers
# sub-menu -- confirmed live against a real v7 router; this exact syntax
# does NOT exist on v6, which is why this is a separate file rather than
# an in-script version branch: confirmed separately that RouterOS v6's
# script engine hard-fails at *run* time on unrecognized v7 command
# paths even inside a branch that's never taken, the same failure class
# already documented for /ping ... as-value).
/system ntp client set enabled=yes mode=unicast
:if ([:len [/system/ntp/client/servers/find where address="203.89.31.10"]] = 0) do={
    /system/ntp/client/servers/add address="203.89.31.10"
}
:if ([:len [/system/ntp/client/servers/find where address="111.68.26.3"]] = 0) do={
    /system/ntp/client/servers/add address="111.68.26.3"
}
...
# always reapply.
#
# require-message-auth=no is v7-only (confirmed live -- this property
# doesn't exist on v6 radius entries at all, hence it's only set here,
# not in the v6 script). Needed because v7's default
# (yes-for-request-resp) rejects login against these RADIUS servers.
# Set/re-set on every run, not just on first add, so a router that
# already had these entries (from the legacy hand-applied config, or
# from an earlier run of this same script before this fix) gets
# corrected too -- not just newly-added entries.
/user aaa set use-radius=yes
:local r1 [/radius find where address="112.78.33.172"]
:if ([:len $r1] = 0) do={
    /radius add address=112.78.33.172 secret=$radiusSecret1 service=login require-message-auth=no
} else={
    /radius set $r1 require-message-auth=no
}
:local r2 [/radius find where address="119.2.52.27"]
:if ([:len $r2] = 0) do={
    /radius add address=119.2.52.27 secret=$radiusSecret2 service=login require-message-auth=no
} else={
    /radius set $r2 require-message-auth=no
}
